...
class TestDashboardBlock(SeleniumXBlockTest):
    ...
    def setUp(self):
        ...
        patches = (
            ...
            ("mentoring.dashboard.DashboardBlock.get_mentoring_blocks", get_mentoring_blocks),
            ("mentoring.dashboard.sub_api", mock_submisisons_api),
            ("mentoring.mcq.sub_api", mock_submisisons_api)
        )
        for p in patches:
            patcher = patch(*p)
            patcher.start()
    ...
    def test_dashboard(self):
        ...
        for step_num, step in enumerate(steps):
            ...
            left_col = avg_row.find_element_by_css_selector('td:first-child')
            self.assertEqual(left_col.text, "Average")
            right_col = avg_row.find_element_by_css_selector('td:last-child')
